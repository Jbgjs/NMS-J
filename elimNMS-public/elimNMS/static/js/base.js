...
function processResults(callback, results) {
  if (results === false) {
    notify("오류 403 - 권한이 없습니다.", "error", 5);
    ...
    for (const [field, error] of Object.entries(results.errors)) {
      notify(`"${field}"에 대한 잘못된 입력: ${error}`, "error", 20);
    }
  } else {
...
function buildBulkFilterPanel(panel, type, formType, tableId) {
  ...
  const filteringFunction = tableId
    ? `lim.table.refreshTable('${tableId}', true, true)`
    : `lim.visualization.displayNetwork()`;
...
function updateProperty(instance, el, property, value, type) {
  ...
  if (propertyType.includes("bool")) {
    ...
    value.forEach((o) => {
      ...
      onclick="lim.base.showInstancePanel('${o.type}', '${o.id}')">
  ...
  $("#alerts").empty().append(`
    ...
    <li style="margin: 3px 6px 0; padding: 10px; margin-bottom: 6px;">
      <div class="text-center">
        <a class="dropdown-item" onclick="lim.base.showAllAlerts()">
    ...
    <li style="margin: 3px 6px 0; padding: 10px; margin-bottom: 6px;">
      <div class="text-center">
        <a class="dropdown-item" onclick="lim.base.clearAlerts()">
...
export function configureNamespace(namespace, functions) {
  lim[namespace] = {};
  functions.forEach((f) => (lim[namespace][f.name] = f));
}
...
function initSidebar() {
  ...
  let switchMenu = function() {
    if ($("body").hasClass("nav-sm")) {
      $("#lim").css({ "font-size": "17px" });
      $("#lim-version").css({ "font-size": "15px" });
      $("#sidebar-menu")
        .find("li.active ul")
        .hide();
      $("#sidebar-menu")
      ...
      $("#lim").css({ "font-size": "30px" });
      $("#lim-version").css({ "font-size": "20px" });
      $("#sidebar-menu")
