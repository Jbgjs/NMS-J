...
function createLabel() {
  if (!instance) return notify(`아직 ${type}이(가) 생성되지 않았습니다.`, "error", 5);
...
function openDeletionPanel() {
  ...
  if (!nodeSelection && !edgeSelection) {
    notify("삭제할 항목이 선택되지 않았습니다.", "error", 5);
  } else {
                ...
                default: function(el, node) {
                  ...
                  <div style="position: absolute; top: 0px; right: 20px">
                    <button
                      ...
                      data-tooltip="Find"
                      onclick='elim.builder.highlightNode(${JSON.stringify(node.data)})'
                    >
