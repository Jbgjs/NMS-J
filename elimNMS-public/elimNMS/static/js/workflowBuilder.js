  ...
  manipulation: {
    ...
    addEdge: function(data, callback) {
      if (data.from.length == 36 || data.to.length == 36) {
        notify("라벨을 사용하여 엣지를 그릴 수 없습니다.", "error", 5);
      } else if (data.to == startId) {
        notify("'시작'으로 엣지를 그릴 수 없습니다.", "error", 5);
      } else if (data.from == endId) {
        notify("'끝'에서 엣지를 그릴 수 없습니다.", "error", 5);
...
function updateWorkflowService(service) {
  ...
  notify(`서비스 '${service.scoped_name}'이(가) 워크플로에 추가되었습니다.`, "success", 5);
}

function addServicesToWorkflow() {
  const selection = $("#service-tree").jstree("get_checked", true);
  if (!selection.length) notify("선택된 항목이 없습니다.", "error", 5);
...
function stopWorkflow() {
  if (!currentRun) notify("워크플로가 현재 실행 중이 아닙니다.", "error", 5);
  const stop = function() {
    call({
      url: `/stop_run/${currentRun.runtime}`,
      callback: (result) => {
        if (!result) {
          notify("워크플로가 현재 실행 중이 아닙니다.", "error", 5);
        } else {
...
function runWorkflow(parametrization) {
  if (isSuperworkflow) {
    return notify("상위워크플로는 직접 실행할 수 없습니다.", "error", 5);
  }
