...
function displayFiles() {
  if ($("#files").length || page == "file_table") {
    return notify("파일 테이블이 이미 표시되어 있습니다.", "error", 5);
  }
  openPanel({
    name: "files",
    size: "1000 600",
  ...
  `/files${folderPath}`
    ...
    .forEach((folder) => {
      ...
      htmlPath.push(`<b> / </b>
        ...
        onclick="elimNMS.administration.enterFolder({path: '${currentPath}'})">
          ${folder}
        </button>
      `);
    });
  $("#current-folder-path").html(`<b>현재 폴더 :</b>${htmlPath.join("")}`);
}
...
function runDebugCode() {
  call({
    ...
    callback: function(result) {
      ...
      notify("코드가 성공적으로 실행되었습니다.", "success", 5, true);
    },
  });
}
...
function migrationsExport() {
  notify("마이그레이션 내보내기가 시작되었습니다.", "success", 5, true);
  call({
    url: "/migration_export",
    form: "migration-form",
    callback: function() {
      notify("마이그레이션 내보내기가 성공했습니다.", "success", 5, true);
    },
  });
}
