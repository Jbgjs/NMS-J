...
function drawDiagrams(type, objects, property) {
  ...
  if (Object.keys(result.data).length > 100) {
    return notify(`${type}s "${property}"에 대해 표시할 데이터가 너무 많습니다.`, "error", 5);
  }
    ...
    content: `
      <div class="modal-body">
        ...
        >
          ...
          <button
            onclick="elimNMS.inventory.downloadNetworkData('${id}', '${name}')"
            type="button"
                ...
                commits[configurationProperty].forEach((commit) => {
                  table.row.add([
                    ...
                    `<button
                      ...
                      onclick="elimNMS.inventory.showGitConfiguration(
...
function exportTopology() {
  notify("토폴로지 내보내기를 시작합니다...", "success", 5, true);
  call({
    url: "/topology_export",
    form: "excel_export-form",
    callback: function() {
      notify("토폴로지가 성공적으로 내보내졌습니다.", "success", 5, true);
    },
  });
}

function importTopology() {
  notify("토폴로지 가져오기: 시작 중...", "success", 5, true);
