...
class Server(Flask):
    ...
    def __init__(self):
        static_folder = str(vs.path / "elimNMS" / "static")
    ...
    def register_plugins(self):
        for plugin, settings in vs.plugins_settings.items():
            try:
                module = import_module(f"elimNMS.plugins.{plugin}")
